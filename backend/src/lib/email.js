...
function orderCollectedEmail({ sellerName, orderId, amount }) {
    return {
...
function payoutReleasedEmail({ sellerName, amount, orderId }) {
    return {
 ...
 */
function buyerReceiptEmail({ buyerName, orderId, productName, quantity, unitPrice, totalAmount, collectionDate, storeName }) {
 ...
 */
function sellerWalletCreditEmail({ sellerName, amount, newBalance, orderId, creditType = 'PAYOUT' }) {
    const yr = new Date().getFullYear()
 ...
 */
function buyerVerifiedEmail({ name }) {
    const yr = new Date().getFullYear()
    return {
 ...
 */
function buyerRejectedEmail({ name, reason }) {
    const yr = new Date().getFullYear()
    return {
 ...
 */
function productApprovedEmail({ sellerName, productName }) {
    const yr = new Date().getFullYear()
    return {
 ...
 */
function productRejectedEmail({ sellerName, productName, reason }) {
    const yr = new Date().getFullYear()
    return {
 ...
 */
function sellerNewOrderEmail({ sellerName, orderId, productName, amount, quantity, collectionDate, token, buyerName }) {
    const yr = new Date().getFullYear()
    const shortId = orderId
    return {
 ...
 */
function rescheduleRequestEmail({ recipientName, proposedDate, proposedBy, orderId }) {
    const yr = new Date().getFullYear()
    const shortId = orderId
    return {
 ...
 */
function rescheduleAcceptedEmail({ recipientName, confirmedDate, orderId }) {
    const yr = new Date().getFullYear()
    const shortId = orderId
    return {
...
module.exports = { sendEmail, orderConfirmationEmail, verificationCodeEmail, orderCollectedEmail, payoutReleasedEmail, welcomeEmail, buyerReceiptEmail, sellerWalletCreditEmail, buyerVerifiedEmail, buyerRejectedEmail, productApprovedEmail, productRejectedEmail, sellerNewOrderEmail, rescheduleRequestEmail, rescheduleAcceptedEmail };
