...
export default function CheckoutModal({ isOpen, onClose, product, quantity = 1, selectedDate }) {
    ...
    // SUMMARY | PAYMENT_DETAILS | PROCESSING | SUCCESS | FAILED
    const [isLoading, setIsLoading] = useState(false)
    const [orderResult, setOrderResult] = useState(null)
    const [senderName, setSenderName] = useState('')
    const [selectedPaymentMethod, setSelectedPaymentMethod] = useState('MANUAL_TRANSFER')
    const [showSenderNamePrompt, setShowSenderNamePrompt] = useState(false)
    ...
    const executeManualTransfer = async (nameToUse) => {
        setIsLoading(true)
        setStep('PROCESSING')
    ...
    const handleSubmitManualTransfer = async () => {
        ...
        if (!name) {
            setShowSenderNamePrompt(true);
            return;
        }

        executeManualTransfer(name);
    }
    ...
    return (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-6 bg-slate-950/60 backdrop-blur-sm animate-in fade-in duration-300">
                    ...
                    {step === 'PAYMENT_DETAILS' && (
                        <div className="space-y-4">
                            ...
                            <div className="space-y-1.5">
                                <label className="text-[9px] font-semibold text-white uppercase tracking-wider block">Sender Account Name</label>
                                <input
                                    type="text"
                                    value={senderName}
            ...
            {/* Custom Sender Name Prompt Popup */}
            {showSenderNamePrompt && (
                <div className="fixed inset-0 z-[80] flex items-center justify-center p-6 bg-slate-950/80 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="bg-[#0c101b] border border-slate-800 rounded-sm w-full max-w-sm p-6 sm:p-8 shadow-2xl space-y-4">
                        <div>
                            <h3 className="font-bold text-white text-xs uppercase tracking-wider">Sender Name Required</h3>
                            <p className="text-slate-400 text-[11px] mt-1 leading-relaxed">
                                Please specify the sender's account name to complete the transfer verification.
                            </p>
                        </div>
                        <input
                            type="text"
                        ...
                        <div className="flex gap-2 pt-2">
                            <button
                                onClick={() => setShowSenderNamePrompt(false)}
                                className="flex-1 py-2.5 rounded-sm font-semibold text-[10px] uppercase tracking-wider text-slate-400 hover:text-white transition-colors bg-[#111625] hover:bg-[#111625]/80 border border-slate-700"
                            >
                                Cancel
                            </button>
                            <button
                                onClick={() => {
                                    ...
                                    setShowSenderNamePrompt(false);
                                    executeManualTransfer(senderName.trim());
                                }}
                                className="flex-1 py-2.5 bg-[#05DF72] hover:bg-[#04c865] text-slate-950 font-semibold text-xs uppercase tracking-wider rounded-sm transition-colors"
                            >
                                Confirm
                            </button>
                        </div>
