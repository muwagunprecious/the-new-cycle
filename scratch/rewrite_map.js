...
const mapStart = '{orders.map((order) => (';
const mapEnd = '))}';

const startIndex = content.indexOf(mapStart);
// Find the correct closing map brace
let depth = 0;
let endIndex = -1;
for (let i = startIndex; i < content.length; i++) {
    if (content.substr(i, 2) === '{(' || content[i] === '{') depth++;
    if (content.substr(i, 2) === ')}' || content[i] === '}') depth--;
    if (depth === 0 && i > startIndex + 10) {
        endIndex = i;
        break;
    }
}
...
if (startIndex !== -1 && endIndex !== -1) {
    const newMapContent = `{orders.map((order) => (
    ...
    content = before + newMapContent + after;
    fs.writeFileSync(path, content);
    console.log('Successfully rewrote the map section with perfectly balanced tags.');
} else {
