...
const SENDER_ID = "talert"; // Termii's universal transactional sender ID
const BASE_URL = "https://api.ng.termii.com";

async function testSendTokenTalertDND() {
    const url = `${BASE_URL}/api/sms/otp/send`;
    console.log(`Sending Token to ${PHONE} using Termii API (${url}) with sender "talert" on DND channel...`);
    ...
    try {
        ...
        if (data.pinId || data.pin_id) {
            console.log("SUCCESS: Token sent via Talert + DND!");
        } else {
